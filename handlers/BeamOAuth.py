...
def beamauth(account):
    ...
    if db.get_db('account_' + account, 'access_token')[1] is None:        
        ...
        if datetimemnow > datetimeexpire:
            ...
            beam_data = beam_auth.generate(db.get_db('account_' + account, 'refresh_token')[1])
...
def initialize():
    ...
    for account in data:
        ...
        if account == 'main' and db.get_db('account_bot', 'access_token')[1] is None:
            print('    Please login to your main Beam account')
            input('    Then press ENTER to continue...')
        # if there is not bot account info then provide user instruction
        elif account == 'bot' and db.get_db('account_bot', 'access_token')[1] is None:
            print('    Please log out of your main Beam account and log into your Beam bot account')
            input('    Then press ENTER to continue...')
        # run account check
        beamauth(account)
